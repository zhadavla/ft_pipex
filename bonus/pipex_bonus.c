...
/*   Updated: 2023/08/08 20:02:09 by mnurlybe         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	execute_command(t_pipex *pipex, int num_of_commands,
		char **list_of_commands, char **env)
{
	int	j;
	int	is_error;
	...
	if ((pipex->infile_name && pipex->infile_fd == -1) || pipex->outfile_fd ==
		-1)
	{
	...
	{
		if (j == 0)
			is_error = do_fork_names(pipex, FIRST_FORK, list_of_commands[j],
				env);
		else if (j == num_of_commands - 1)
			is_error = do_fork_names(pipex, LAST_FORK, list_of_commands[j],
				env);
		else
		{
